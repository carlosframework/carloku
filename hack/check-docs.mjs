...
//    Ported from the website's check-docs.mjs, which paid for this the
//    hard way: a passthrough file list going stale there dropped a live
//    page and shipped a 404ing stylesheet, both silently, because
//    npm run check only ever looked under _site/docs and exited 0
//    either time. A build missing its homepage should fail before
//    anyone reads about anchors, so this runs first.
...
//
// The "//" key is not a case: it is generated-by metadata some fixture
// writers prepend to every JSON file they emit. Skip it by name rather
// than by loosening the comparison — the strictness IS this file's
// purpose.
